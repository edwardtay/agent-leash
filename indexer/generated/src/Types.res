...
type loaderContext = {
  ...
  @as("DailyStats") dailyStats: entityLoaderContext<Entities.DailyStats.t, Entities.DailyStats.indexedFieldOperations>,
  @as("VaultDeposit") vaultDeposit: entityLoaderContext<Entities.VaultDeposit.t, Entities.VaultDeposit.indexedFieldOperations>,
  @as("VaultWithdrawal") vaultWithdrawal: entityLoaderContext<Entities.VaultWithdrawal.t, Entities.VaultWithdrawal.indexedFieldOperations>,
}
...
type handlerContext = {
  ...
  effect: 'input 'output. (Envio.effect<'input, 'output>, 'input) => promise<'output>,
  chains: Internal.chains,
  @as("DailyStats") dailyStats: entityHandlerContext<Entities.DailyStats.t>,
  @as("VaultDeposit") vaultDeposit: entityHandlerContext<Entities.VaultDeposit.t>,
  @as("VaultWithdrawal") vaultWithdrawal: entityHandlerContext<Entities.VaultWithdrawal.t>,
}

//Re-exporting types for backwards compatability
@genType.as("DailyStats")
type dailyStats = Entities.DailyStats.t
@genType.as("VaultDeposit")
type vaultDeposit = Entities.VaultDeposit.t
@genType.as("VaultWithdrawal")
type vaultWithdrawal = Entities.VaultWithdrawal.t
...
module SimpleVault = {
let abi = Ethers.makeAbi((%raw(`[{"type":"event","name":"Deposit","inputs":[{"name":"user","type":"address","indexed":true},{"name":"amount","type":"uint256","indexed":false},{"name":"timestamp","type":"uint256","indexed":false}],"anonymous":false},{"type":"event","name":"Withdrawal","inputs":[{"name":"user","type":"address","indexed":true},{"name":"amount","type":"uint256","indexed":false},{"name":"timestamp","type":"uint256","indexed":false}],"anonymous":false}]`): Js.Json.t))
let eventSignatures = ["Deposit(address indexed user, uint256 amount, uint256 timestamp)", "Withdrawal(address indexed user, uint256 amount, uint256 timestamp)"]
@genType type chainId = [#84532 | #11155111]
let contractName = "SimpleVault"
...
@genType
type eventArgs = {user: Address.t, amount: bigint, timestamp: bigint}
@genType
type block = Block.t
@genType
...
let paramsRawEventSchema = S.object((s): eventArgs => {user: s.field("user", Address.schema), amount: s.field("amount", BigInt.schema), timestamp: s.field("timestamp", BigInt.schema)})
...
let register = (): Internal.evmEventConfig => {
  let {getEventFiltersOrThrow, filterByAddresses} = LogSelection.parseEventFiltersOrThrow(~eventFilters=handlerRegister->EventRegister.getEventFilters, ~sighash, ~params=["user",], ~topic1=(_eventFilter) => _eventFilter->Utils.Dict.dangerouslyGetNonOption("user")->Belt.Option.mapWithDefault([], topicFilters => topicFilters->Obj.magic->SingleOrMultiple.normalizeOrThrow->Belt.Array.map(TopicFilter.fromAddress)))
  {
    ...
    convertHyperSyncEventArgs: (decodedEvent: HyperSyncClient.Decoder.decodedEvent) => {user: decodedEvent.indexed->Js.Array2.unsafe_get(0)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, amount: decodedEvent.body->Js.Array2.unsafe_get(0)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, timestamp: decodedEvent.body->Js.Array2.unsafe_get(1)->HyperSyncClient.Decoder.toUnderlying->Utils.magic, }->(Utils.magic: eventArgs => Internal.eventParams),
    id,
  name,
  contractName,
...
let id = "0xdf273cb619d95419a9cd0ec88123a0538c85064229baa6363788f743fff90deb_2"
let sighash = "0xdf273cb619d95419a9cd0ec88123a0538c85064229baa6363788f743fff90deb"
let name = "Withdrawal"
let contractName = contractName
